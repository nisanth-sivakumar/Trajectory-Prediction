...
class KalmanFilter:
    def __init__(self, dt=1/30, process_var=1.0, meas_var=10.0):
        # state: [x, y, vx, vy]
        self.dt = dt
        self.F = np.array(
            [[1, 0, dt, 0],
        ...
        # self.Q = np.eye(4) * process_var # noise
        ...
        self.x = np.zeros((4, 1)) # init state
    ...
    def predict(self):
        ...
        return self.x[:2].flatten() # returns only (x, y)

    def update(self, z): # z = (x, y)
        z = np.array(z).reshape(2, 1)
        y = z - self.H @ self.x # residual
        S = self.H @ self.P @ self.H.T + self.R
        K = self.P @ self.H.T @ np.linalg.inv(S) # k gain
        self.x = self.x + K @ y
        self.P = (np.eye(4) - K @ self.H) @ self.P
        return self.x[:2].flatten() # updated (x, y)
...
def draw_boxes(frame, bbox_xyxy, draw_trails, identities=None, categories=None, offset=(0,0)):
    ...
    for i, box in enumerate(bbox_xyxy):
        ...
        if id not in kf_dict:
            kf_dict[id] = KalmanFilter()
            kf_dict[id].initiate(center[0], center[1])
        else:
            # update measurement
            pred_xy = kf_dict[id].predict()
            kf_dict[id].update(center)
        
        # predictions
        future_points = []
        kf_copy = KalmanFilter()
        kf_copy.x, kf_copy.P = kf_dict[id].x.copy(), kf_dict[id].P.copy()
        for _ in range(10):
            pred_xy = kf_copy.predict()
            future_points.append((int(pred_xy[0]), int(pred_xy[1])))
